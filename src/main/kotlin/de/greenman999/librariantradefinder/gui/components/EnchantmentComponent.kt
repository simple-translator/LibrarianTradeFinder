...
class EnchantmentComponent(val entry: MutableMap.MutableEntry<Identifier, Config.EnchantmentEntry>, searchTextState: BasicState<String>, fuzzyScore: FuzzyScore) : UIRoundedRectangle(4f) {
	...
	fun reset() {
		entry.setValue(Config.EnchantmentEntry(
			false,
			RegistryHelper.getEnchantmentById(entry.key).maxLevel,
			RegistryHelper.getMaxEmeraldCost(entry.key, RegistryHelper.getEnchantmentById(entry.key).maxLevel)
		))
		updateEntry(false)
	}
}
